...
- `origin` (HTTPS) — works without SSH keys.
- `upstream` (SSH) — requires a working SSH key in WSL; otherwise you’ll get `Permission denied (publickey)`.

Recommended push from PowerShell (no UNC paths):
- `wsl -d Ubuntu bash -lc 'cd ~/Norskkurs && git push origin main'`

If `git push` fails because the branch tracks `upstream/main`, switch tracking to HTTPS remote once:
- `wsl -d Ubuntu bash -lc 'cd ~/Norskkurs && git branch --set-upstream-to=origin/main main'`

Then plain `git push` will push to `origin/main`.

### Work report (2025-12-26)
- Added Sentry SDK for backend and frontend with env-driven init and .env.example keys.
- Hooked OpenAPI schema + generated types into frontend API helpers.
- Added WordCollapse 2 game (separate storage keys) and allowed selecting blocks while falling.
- Added OpenAPI docs routes on backend (/api/schema/, /api/docs/, /api/redoc/).
- Added GlitchTip test triggers: backend /api/debug/sentry-test/ (DEBUG only) and frontend ?sentry_test=1 gated by VITE_SENTRY_TEST.
- Tests/builds: `docker compose exec backend python manage.py test exams --keepdb`; `docker compose build frontend`; `docker compose exec frontend npm run build`; restarted backend/frontend.
...
## Source: tests demo refactor (2025-12-16)

#### Summary
- Вкладка **Test** полностью переведена на фронтенд‑демо вместо старого списка реальных тестов.
- Добавлен красивый одностраничный демо‑тест с прогресс‑баром, клавиатурными шорткатами (Enter/Esc), drag‑and‑drop‑заданием и разбором ответов.
- Вопросы теперь зависят от **stream** (Bokmål / Nynorsk / English) и **уровня** (A1–B2).

#### Details
- `frontend/src/pages/TestsPage.tsx`:
  - Старый API‑driven интерфейс удалён; страница стала чисто демо‑тестом.
  - Вопросы собираются функцией `buildDemoQuestions(stream, level)`:
    - Для **Bokmål/Nynorsk**: норвежские примеры по уровням A1–B2 (приветствия, рутина, придаточные, формальный/официальный стиль).
    - Для **English‑stream**: отдельные английские наборы по уровням A1–B2 (simple present, daily routine, connectors `because/although`, first conditional и т.п.).
    - Учитываются формы `Jeg/Eg`, `heter/heiter`, `lager/lagar`, `Selv/Sjølv`, `hverdag` и др.
  - Сохранён прежний проп‑интерфейс `TestsPage`, чтобы `App.tsx` не трогать (мы по‑прежнему используем `stream`, `currentLevel`, `levelLabel`, профиль).
  - Шапка демо‑теста показывает человекочитаемые бейджи: локализованный уровень (`levelLabel`) и название потока (`streamLabels.bokmaal/nynorsk/english`).
  - Логика:
    - Фазы: `intro` → `running` → `result`.
    - Вопросы трёх типов: `single` (один вариант), `fill` (ввод текста), `order` (перетаскивание слов мышкой).
    - Поддержка Enter (проверка/Next) и Esc (выход из теста).
    - Подсчёт баллов по каждому вопросу и итоговый процент + таблица обзора (ваш ответ/правильный ответ).
- `frontend/src/i18n.ts`:
  - Добавлен блок `testsDemo` для EN/NB/RU: заголовок, подзаголовок, подсказки прогресса, подписи для результатов, кнопки (`Start demo`, `Shuffle`, `Check`, `Finish`, `Review`, и т.д.).
  - Используются те же `streamLabels`/`levelLabel`, что и в остальном приложении.
- `frontend/src/style.css`:
  - Добавлен блок стилей `demo-test-*`:
    - Герой‑карточка демо‑теста с градиентным фоном, бейджами потока/уровня.
    - Прогресс‑бар, карточка вопроса с подсветкой правильного/неправильного состояния.
    - Кнопки‑варианты, поле ввода, “чипы” для drag‑&‑drop предложения.
    - Блок результата с бейджами процента и баллов, список вопросов‑разборов.

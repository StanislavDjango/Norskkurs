...
import { fetchProfileProgress, fetchTestDetail, fetchTests, submitTest } from "../api";
import type {
  AnswerPayload,
  Level,
  ProfileInfo,
...
type FilterMode = "all" | "single" | "fill" | "mixed" | "exam";
    ...
    <>
      ...
      <div className="layout">
        <aside className="panel">
          <h2>{t("selectTest")}</h2>
          {!isTeacher && <div className="student-auth-card">{renderAuthFields()}</div>}
          ...
          <div className="test-list">
            ...
            {visibleTests.length === 0 && <div className="muted small">{t("noTests")}</div>}
          </div>
  ...
  ({ question, answer, missing, review, onSelectOption, onChangeText }, ref) => {
    ...
    return (
      <article className={`question ${statusClass}`} ref={ref}>
        ...
        {missing && !review && <div className="question-hint">{t("answerRequired")}</div>}
      </article>
    );
  },
);

...
3. Seed data is automatic; admin UI at `http://localhost:8000/admin/` (create a superuser via `docker compose run --rm backend python manage.py createsuperuser`).
4. Frontend runs at `http://localhost:5173` (uses API `http://backend:8000/api/` inside the compose network).
